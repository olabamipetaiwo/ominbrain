...
## 2026-09-25 (evening) — bf16 precision control finished; added to stats, paper numbers and paper

**Run.** Smoke 43323437 (2 patients, exit 0) then full 43323485: COMPLETED, exit 0, 57 min on one RTX PRO 6000; 525 records (62 patients x own/text/swap x AIA/LIL/DSCR, LIL only 51), 0 parse errors, `results/lumiere_v4_Gemma-3-27B-bf16_{smoke,image}_20260925_*`. The 18 smoke records are identical to the full run's for patients 1-2 (temperature 0), so merging both folders in the stats tool changes nothing. The smoke scored 0/18, which looked alarming but was two patients only; the full run scores normally.
**Code.** `tools/lumiere_v4_stats.py`: `CONTROLS = ["Gemini-3.6-Flash", "Gemma-3-27B-bf16"]` (E1/E2 only, outside the Holm family; heading of the control section renamed). `tools/paper_numbers.py`: `TAG["Gemma-3-27B-bf16"] = "GemmaTwentySevenBf"`, so the existing control loop emits the same macro families (`vfourEone...GemmaTwentySevenBf`, `vfourEtwo...GemmaTwentySevenBf`); no new table (`tab_v4_ctrl.tex` is still Gemini only). Checked: the four registered models' and Gemini's blocks in `lumiere_v4_stats.json` are byte-equal to before; `paper_numbers --check` finds no stale numbers (467 macros/tables).
**Results (n = 62 AIA / 51 LIL / 62 DSCR; bf16 vs the 4-bit run of the same model).**
- E1 AIA: own 33.9 vs text-only 25.8, +8.1 [-3.2, +19.4] (9 only-own / 4 only-text), inconclusive. 4-bit: 32.3 vs 25.8, +6.5 [-4.8, +17.7]. Gemini control: 93.5 vs 24.2.
- E1 LIL: 25.5 vs 25.5, 0.0 [-13.7, +13.7]. E1 DSCR: 56.5 vs 56.5, 0.0 [0.0, 0.0] (= the majority-key share; every answer is progressive disease with or without the image).
- E2 donor tracking: AIA gain +11.3 [0.0, +22.6] (swap gives the donor's key 33.9%); LIL +2.0 [-11.8, +15.7]; DSCR +3.2 [0.0, +8.1] (a swap changes 4.8% of DSCR answers).
- **Reading under the rule fixed in the preregistration change log:** the AIA interval includes 0 and accuracy stays near chance (25%), so quantization does NOT explain the gap to the control; bf16 and 4-bit are indistinguishable (33.9 vs 32.3). Not "above and clearly above the 4-bit model's", so the "partly quantization" branch does not apply.
**Caveats (stated in the paper).** One model, one run; vLLM vs Ollama stack differs, so precision is confounded with the stack; says nothing about the other three open models (Llama-4-Scout does not fit one GPU in bf16); not clinician-validated items.
**Paper (`paper/latex/acl_latex.tex`).** Section 4 "Control model" paragraph: two sentences disclosing the precision control and its reading rule; Results: new "Precision control" paragraph (all numbers macros) before "Open models: donor tracking"; Limitations: one clause on the bf16 rerun. Compiles clean on a scratch copy (0 errors, 0 undefined/multiply-defined, 12 pages).
**Follow-up edit (user asked, after committing):** "What this shows and does not show" now says quantization is separated only for Gemma-3-27B (the other three ran at 4-bit only) and no longer lists 4-bit as an open confound for the gap in general; the Conclusion says scale and training are not separated and 4-bit serving only for Gemma-3-27B. Recompiles clean (12 pp, 0 errors/undefined). Limitations already carried the one-model scope. The next paragraph is the pre-edit note, now done:
**Appendix A hand-typed numbers replaced by macros (user asked, later).** New `v3_appendix()` in `tools/paper_numbers.py` (+44 macros, 511 total; `--check` clean) computes from the result files: text-only same-answer share (recomputed from the `v3_nogate` and `textonly_v3_nogate` runs), repeat identical answers (asserts all equal n), own-vs-donor flip ranges (`lumiere_counterfactual_summary.json`), evidence-audit shares (`lumiere_reasoning_facts.csv`, `<model>_v3` rows), options-only ranges (cells flagged for parse errors are excluded, as the text says), MedGemma unparsed share, option heuristics (`baselines`), max |text-only diff|, and `permObservedPct`.
Every previous value reproduced exactly EXCEPT one error: the LIL options-only range was typed "33 to 37%"; the data give 29 to 37% (MedGemma-4B 28.8%, unflagged, had been left out). Now generated, so the text reads 29 to 37. Wording change in the evidence-audit paragraph: the hand-typed "contradict" shares (10 to 18, 2 to 6) are contradicted PLUS unmatched claims (a measurement matching no fact value; contradicted alone is 0.7 to 12.6 and 0.3 to 4.8), so the text now says "contradict it or match no record value".
Still typed by hand (not in a results file, or pipeline facts): construction counts (91/55/54/269), adaptation "1 to 4 of 52" (Appendix B), "0 of 52 LIL patients" donor-answer-in-options, the worked-example values (Patient-002), the option-only "$n=52$", and the $\pm8$pp margin (a design choice). Not checked against data.
**Not changed, worth a look (superseded by the follow-up above):** the "What this shows and does not show" paragraph and the Conclusion still say the open-model gap mixes scale and "4-bit serving"; with one bf16 result the claim about quantization could be narrowed to "for Gemma-3-27B". Preregistration change log has no results entry (by design it holds plans only). Still open: clinician review; everything uncommitted.
...
**Status check:** `bash tools/v4_status.sh`. At 12:55 on 2026-09-24 the three image jobs (43199273, 43199276, 43199279) had run about an hour; nothing finished; 9 jobs queued behind them. SLURM mails END/FAIL (address set in the sbatch file). If a job FAILS, its slot's next job still starts (afterany); rerun the failed one with `sbatch --job-name=lumiv4_<mode>_<model> --export=ALL,MODELS=<model>,MODE=<mode> shell/lumiere/lumiere_v4.sbatch`.
