...
\title{Right Answer, No Image: Shortcut Learning in Brain-MRI Question Answering}
...
\maketitle
\begin{abstract}
Medical multimodal LLMs are usually scored on final-answer accuracy, which cannot show whether an answer came from the image, the question text, or a shortcut. We study this on brain MRI with \textbf{Knowledge Alignment Benchmarking (KAB)}, which evaluates a case as a sequential five-phase chain. OmniBrainBench's released groupings cannot support same-patient chains (only 2 verifiable cross-phase patient links among 6,823 questions), so we build a same-patient chain dataset from the LUMIERE glioblastoma cohort (52 patients) and run controls on four open-weight models. Accuracy is largely explained by non-image information. Text-only accuracy is within about $\pm8$ points of image-present accuracy (a margin set post hoc); substituting another patient's image does not move answers toward that patient's label beyond a permutation null ($p=.64$); and treatment-management answers depend on the upstream diagnosis label: correct versus wrong diagnosis context changes accuracy by 17--39 points, and the diagnosis entry alone reproduces the effect in three of four models. Items and answer keys are not clinician-validated, so the results are exploratory.
\end{abstract}

\section{Introduction}
\label{sec:introduction}

Multimodal large language models (MLLMs) are increasingly applied to medical imaging, yet the dominant evaluation treats medical visual question answering like any multiple-choice benchmark and scores only the selected option. Final-answer accuracy cannot separate a model that read the image and reasoned validly from one that pattern-matched cues in the question or options, or guessed. Such shortcuts are well documented outside medicine: hypotheses alone suffice to beat chance on natural language inference \citep{gururangan2018annotation}, models adopt syntactic heuristics that fail under adversarial evaluation \citep{mccoy2019right}, and stated chain-of-thought often does not reflect what produced the answer \citep{turpin2023language}. Medical multimodal models often answer from linguistic patterns or irrelevant image regions rather than the pathology a question asks about \citep{nguyen2025localizing}. Clinically, an accurate answer that does not depend on the image cannot be trusted on the next case that differs slightly.

We ask whether accuracy on brain-MRI multiple-choice questions reflects reading the image or a shortcut. To test this we use \textbf{Knowledge Alignment Benchmarking (KAB)}, which evaluates a brain MRI case as a sequential chain of the five clinical phases of the OmniBrainBench workflow \citep{peng2026omnibrainbench}: \textbf{A}natomical and \textbf{I}maging \textbf{A}ssessment (AIA), \textbf{L}esion \textbf{I}dentification and \textbf{L}ocalization (LIL), \textbf{D}iagnostic \textbf{S}ynthesis and \textbf{C}ausal \textbf{R}easoning (DSCR), \textbf{P}rognostic \textbf{J}udgment and \textbf{R}isk \textbf{F}orecasting (PJRF), and \textbf{T}herapeutic \textbf{C}ycle \textbf{M}anagement (TCM). Beyond accuracy, KAB scores whether a model's reasoning uses valid clinical vocabulary and is self-consistent (\textbf{vocabulary and consistency}), whether it can use a targeted clinical correction (\textbf{feedback-adaptation}), and whether its answers are jointly correct across phases (\textbf{threshold gating}, a scoring rule rather than a demonstrated dependency). Figure~\ref{fig:overview} summarizes the framework; the controls that carry our conclusions are in Section~\ref{sec:grounding-test}.
...
Building KAB on OmniBrainBench \citep{peng2026omnibrainbench}, we found that the released benchmark cannot support the same-patient sequential evaluation KAB requires: its \texttt{source\_file} grouping, which we had interpreted as patient identity, does not track it. Of 6,823 questions, only 2 patients (4 questions) form a verifiable cross-phase pair, a lower bound since verification relies on shared images or filename-derived identifiers (Section~\ref{sec:chain-validity}). This is an incompatibility between the released benchmark and our evaluation; we are not aware of an explicit claim by OmniBrainBench that its released closed-ended questions form patient-linked chains (its supplement describes a separate, ADNI-based patient-level extension, which we did not use). We therefore build a same-patient chain dataset from LUMIERE \citep{suter2022lumiere}, a 91-patient glioblastoma cohort with longitudinal imaging, expert RANO ratings, survival times for a subset of patients, and treatment information, using LLM drafting and an automatic leakage auditor (Section~\ref{sec:lumiere}).

This paper makes three contributions:
\begin{itemize}[leftmargin=*, itemsep=2pt, topsep=2pt]
    \item Evidence, on four open-weight models, that accuracy on chain-structured brain-MRI questions is largely explained by non-image information (Section~\ref{sec:grounding-test}): text-only accuracy matches image-present accuracy within a post hoc $\pm8$pp margin, image substitution does not move answers toward the donor's label beyond a permutation null, and treatment-management answers depend on the upstream diagnosis label. The findings are exploratory: answerability and answer keys are not clinician-validated (Section~\ref{sec:limitations}), and proprietary models are not run.
    ...
    \item The KAB framework: ontology-based vocabulary scoring with an answer-reasoning consistency probe, feedback-adaptation, and threshold gating, as exploratory instruments (adaptation triggered on only 1--4 of 52 patients per model; the dependency that gating assumes is what the gating-causality experiment tests).
\end{itemize}

\section{Related Work}
\label{sec:related-work}

\paragraph{Shortcuts and reasoning faithfulness.} A model can reach a correct label without doing the intended task: natural language inference hypotheses alone carry enough annotation artifacts to outperform chance \citep{gururangan2018annotation}, and models trained on such data adopt syntactic heuristics that fail on HANS \citep{mccoy2019right}. For chain-of-thought, \citet{turpin2023language} show that adding biasing features to the input, such as reordering the options so the answer is always (A), changes model answers while the models systematically fail to mention the feature in their explanations. KAB's answer-recoverability probe is closest to this line but tests only textual consistency: we withhold everything except the reasoning text and ask whether it reproduces the answer.

\paragraph{Answering without the image.} Hypothesis-only baselines beat the majority class on many NLI datasets \citep{poliak2018hypothesis}. In visual question answering, language priors let models answer without the image, and \citet{goyal2017making} rebalanced the data so that each question has two images with different answers. \citet{chen2024right} find that visual content is unnecessary for many samples in popular multimodal benchmarks. In medicine, \citet{yan2025worse} show that state-of-the-art models score below random guessing on specialized diagnostic questions when ground-truth questions are paired with adversarial counterparts that negate or hallucinate attributes, and \citet{jin2024hidden} find flawed rationales in 35.5\% of GPT-4V's correct answers, most often in image comprehension (27.2\%). Our controls apply this logic to chain-structured questions and add an intervention on the upstream context.

\paragraph{Shortcut tests and phased diagnosis in medical reasoning.} \citet{nguyen2025localizing} introduce textual and visual perturbation tests for shortcut learning in chest-radiograph VQA; their questions are single items, not multi-phase chains. The five phases are OmniBrainBench's clinical workflow \citep{peng2026omnibrainbench}; we treat the dependency between phases as an assumption to be tested (Section~\ref{sec:gating-causality}).

\paragraph{Brain MRI benchmarks and same-patient continuity.} OmniBrainBench \citep{peng2026omnibrainbench} is, to our knowledge, the published brain-imaging MLLM benchmark organized around a multi-phase clinical workflow (15 imaging modalities; 6,823 closed-ended and 2,704 open-ended questions), and is where we started. Its supplement describes a patient-level extension with subject IDs across the five phases, built on the ADNI Alzheimer's cohort; the authors' repository provides label-generation code for it but no question data, and we did not evaluate it. LUMIERE \citep{suter2022lumiere} and MU-Glioma-Post \citep{muglioma2025post} provide real longitudinal data with treatment and outcomes but no question layer. Among published brain-tumor datasets we are aware of, none combines same-patient continuity from imaging to treatment with a multiple-choice layer; Section~\ref{sec:lumiere} builds one on LUMIERE.
...
\section{Controls for Shortcut Use}
...
Gemma-3-27B & 64/25/33 & $+26.9^{*}$ & $+5.8$ & 54/64/71 \\
Llama-4-Scout & 60/42/33 & $+26.9^{*}$ & $+3.8$ & 50/62/64 \\
\bottomrule
\end{tabular}
\caption{Accuracy (\%) under correct (c), forced-wrong (w), and absent (a) upstream context ($n=52$). $\Delta$ is correct minus wrong TCM accuracy in points when only the DSCR entry ($\Delta_{\text{DSCR}}$) or every entry except DSCR ($\Delta_{\text{no DSCR}}$) is injected; $^{*}$ the patient-clustered 95\% interval excludes zero. Full-context TCM correct minus wrong is $+17.3$, $+34.6$, $+38.5$, $+17.3$ points, all excluding zero. Uncorrected.}
\label{tab:gating}
\end{table}

\paragraph{Gating-causality and TCM.} Table~\ref{tab:gating} gives the interventions. Only TCM depends on upstream context: correct context beats forced-wrong context by 17.3--38.5pp for all four models (exact McNemar $p\le.004$, uncorrected), and the DSCR entry alone reproduces the gap for Gemma-3-12B, Gemma-3-27B, and Llama-4-Scout while the other upstream entries do not (all intervals include zero). MedGemma-4B is the exception: neither subset reproduces its full-context gap. LIL and PJRF show no detectable dependence for any model (no paired contrast with $p<.2$). The DSCR key predicts the action class of the TCM key (escalate or continue) in 50 of 52 patients under a fixed keyword rule (Appendix~\ref{sec:app-audit}), and in the own-image run TCM is correct far more often when the model's own DSCR answer was correct (Gemma-3-12B 17/29 versus 3/23, Gemma-3-27B 26/34 versus 5/18, Llama-4-Scout 18/30 versus 1/22; descriptive, since easier patients may be easier throughout). Because the drafting prompt makes TCM follow from the disease course, this dependency is built into the items: TCM accuracy in the three larger models largely reflects a lookup of the upstream diagnosis label and is not evidence of multi-phase reasoning. Separately, correct DSCR context \emph{lowered} DSCR accuracy relative to absent context in three models (Gemma-3-12B $-15.4$pp, $p=.021$; Gemma-3-27B $-17.3$, $p=.012$; Llama-4-Scout $-13.5$, $p=.039$; uncorrected across 48 contrasts). Anchoring on the injected label is a hypothesis we did not test.

\paragraph{Options-only baseline.} Appendix Table~\ref{tab:optonly} gives the results. For the three models with parseable output, options alone score 19--44\% on DSCR (below the 63.5\% majority-label rate), 21--25\% on PJRF, and 12--19\% on TCM (below the 25\% chance level), so the options are not the main shortcut on those phases. On LIL, options alone (33--37\%) score at least as well as the same items shown with image and stem but no chain (23--29\%). AIA is the exception (options-only 38--62\%, own-image 86--94\%), but its answer is the same string in every item. MedGemma-4B's options-only responses were unparseable for 35--100\% of items in four phases (DSCR 52 of 52), so those cells are flagged and not interpreted.
...
\paragraph{Reading.} On this item set and these models, accuracy is largely explained by information other than the image. Text-only accuracy is within $\pm8$pp of image-present accuracy (a margin adopted after inspection; not within $\pm5$pp), novel record-matching claims are 1--2\% of checkable claims, substituted images do not move answers toward the donor's label beyond a permutation null, and the one phase whose accuracy depends on the chain, TCM, depends on it through the upstream diagnosis label. The flip rate is not low, so the third condition of the analysis plan (a low, non-truth-tracking flip rate) is not met as written: answers do change with the image, but not toward the substituted patient's label beyond base rates. We do not conclude that the images are ignored or that the models cannot use them: answerability from the supplied slice is unvalidated, answer keys have not been checked by a clinician, and an item that cannot be answered from the image cannot show image dependence.
...
The results cover the four open-weight models only. The item set is a correction of a leakier earlier draft, and zero auditor violations show compliance with an auditor the drafting loop was optimized against, not absence of leakage. The equivalence margin for the text-only comparison was set after seeing the intervals, the flip rate was not low, so the analysis plan's ``low, non-truth-tracking flip rate'' condition was not met as written, and the donor-permutation, gating-causality, ablation, and options-only analyses were added after seeing earlier results. Intervals treat patients as exchangeable, are not adjusted for multiplicity across the 20 model-phase cells, and share the same 52 patients and donor pairing across models; the 48 gating-causality contrasts are likewise uncorrected. The wrong-context condition uses one fixed incorrect option per phase, so its effect is not averaged over wrong labels; TCM's dependence on DSCR is built into the item drafting; the own-DSCR split of TCM accuracy is confounded by patient difficulty; and MedGemma-4B's options-only responses were largely unparseable and are not interpreted. Ontology term coverage is a string-matching metric against fixed vocabularies that measures term validity, not correctness, and it undercounts correct concepts phrased outside the ontologies; answer recoverability detects only inconsistency between reasoning and answer. The evidence audit is deterministic pattern matching without paraphrase handling and is unvalidated. In OmniBrainBench, PJRF and TCM are 64 and 51 questions, so results there need confidence intervals and must not be pooled; LUMIERE's per-phase intervals are wide at $n=52$, and we report them rather than one pooled number. LUMIERE's tumor segmentations are automated (DeepBraTumIA, HD-GLIO-AUTO) and not manually verified, whereas RANO ratings are expert-assigned, and LUMIERE is a single-center cohort. Finally, no model here needs multi-GPU tensor parallelism, so a larger open-weight model would reopen that question.

\section{Conclusion}
\label{sec:conclusion}

We asked whether accuracy on chain-structured brain-MRI questions reflects reading the image or a shortcut. OmniBrainBench's released groupings cannot support same-patient chains (2 verifiable cross-phase links among 6,823 questions), so we built a five-phase chain dataset from 52 LUMIERE glioblastoma patients and ran controls on four open-weight models. Accuracy was largely explained by non-image information: text-only accuracy stayed within $\pm8$pp of image-present accuracy, substituted images did not move answers toward the donor's label beyond a permutation null, and treatment-management accuracy depended on the upstream diagnosis label, which for three of four models was sufficient on its own. Final-answer accuracy alone would have hidden all three effects. These findings are exploratory. No clinician has validated the items or answer keys, and one 2D slice per timepoint may not make every question answerable from the image, so we do not conclude that the models ignore images. Clinician review of answerability, extension to proprietary models, and multi-slice inputs are the next steps, and the controls themselves (text-only, image substitution, context ablation, options-only) apply to any chain-structured medical benchmark.
...
\bottomrule
\end{tabular}
\caption{Accuracy (\%, $n=52$ per cell) with the options alone; with image and stem but no chain context (the ``absent'' condition of the gating-causality run, and the ordinary own-image run for AIA, which has no upstream phase); and in the ordinary own-image run, which carries the model's own earlier answers as chain context. Chance is 25\%; the majority answer text is 100\% for AIA, 63.5\% for DSCR, and 1.9\% for LIL, PJRF, and TCM. Opt.-only is options only; Own-img is the ordinary own-image run; Unpars. is the number of unparseable options-only responses; unparseable responses score incorrect. $^\dagger$ More than 20\% unparseable: a lower bound, and its paired comparison with the own-image run is suppressed as an artifact. Own-image minus options-only mixes image, stem, and the model's own upstream answers, so it is not an image effect.}
\label{tab:optonly}
\end{table}

\paragraph{TCM label lookup.} A fixed keyword rule assigns each TCM option an action class from its first ten words (escalate: second-line or salvage therapy, re-resection, radiosurgery, re-irradiation, trial; stop: discontinue, suspend, best supportive care; continue: continue, maintain; otherwise other; rules in \texttt{tools/lumiere\_tcm\_lookup.py}). The DSCR key predicts the TCM key's class (progressive disease $\to$ escalate, other ratings $\to$ continue) in 50 of 52 patients (33 of 33 and 17 of 19), but the key's class is unique among the four options in only 18 of 52, so the class alone fixes the letter in a minority of items. Among the 33 patients whose DSCR key is progressive disease, the share of TCM answers in the escalate class under correct, forced-wrong, and absent DSCR context is 24, 13, and 11 of 33 for Gemma-3-12B; 25, 14, and 20 for Gemma-3-27B; 23, 17, and 15 for Llama-4-Scout; and 8, 7, and 8 for MedGemma-4B, which does not follow the injected label. TCM accuracy split by whether the model's own DSCR answer was correct gives Fisher exact $p=.0013$ (Gemma-3-12B), $.0010$ (Gemma-3-27B), $3\times10^{-5}$ (Llama-4-Scout), and $.19$ (MedGemma-4B); these splits are descriptive and confounded by patient difficulty.
