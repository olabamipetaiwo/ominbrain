...
\footnotesize
\setlength{\tabcolsep}{3pt}
\input{generated/tab_v3_textonly}
...
\paragraph{Text-only ablation.} Three of four models score at least as high without the image (Table~\ref{tab:textonly}; Llama-4-Scout \vthreeDiffScout{}, Gemma-3-12B \vthreeDiffGemmaTwelve{}, MedGemma-4B \vthreeDiffMedGemma{} points; Gemma-3-27B \vthreeDiffGemmaTwentySeven{}, mostly AIA). Every interval includes zero and lies within $\pm8$pp. Models give the same answer with and without the image on \vthreeSameLo{} to \vthreeSameHi{}\% of items, so the image changes answers without changing net accuracy. Repeating the own-image run gives identical answers on \vthreeRepeatIdentical{} of \vthreeRepeatN{} questions for all four models, so every change below is caused by the input.

\paragraph{Evidence fact-check.} \vthreeEvNoclaimLo{} to \vthreeEvNoclaimHi{}\% of answers contain no checkable claim. Among the rest (\vthreeEvClaimsLo{} to \vthreeEvClaimsHi{} claims per model), \vthreeEvInOkLo{} to \vthreeEvInOkHi{}\% were available in shown text and match the record, \vthreeEvInBadLo{} to \vthreeEvInBadHi{}\% were available yet contradict it or match no record value (for example a wrong location repeated from an earlier turn), \vthreeEvNewOkLo{} to \vthreeEvNewOkHi{}\% were novel and match, and \vthreeEvNewBadLo{} to \vthreeEvNewBadHi{}\% were novel and contradict it or match no record value. Few novel matching claims means little evidence that stated evidence contains facts obtainable only from the image; it cannot show the converse.

\paragraph{Counterfactual images.} Flip rates between own and donor image are \vthreeFlipAIALo{} to \vthreeFlipAIAHi{}\% for AIA (whose answer is a fixed string), \vthreeFlipLILLo{} to \vthreeFlipLILHi{}\% for LIL and \vthreeFlipDSCRLo{} to \vthreeFlipDSCRHi{}\% for DSCR. Among \permN{} flipped LIL and DSCR items, \permObserved{} (\permObservedPct{}\%) carry the donor's label against a permutation mean of \permMean{} (95\% range \permLo{} to \permHi{}; $p=\permP{}$), so which donor's image was shown does not matter beyond label base rates. The LIL part is degenerate by the pairing rule, and the donor's fully correct answer was among the options for 0 of 52 LIL patients, so the test is limited to label level.

\begin{table}[t]
\centering
\footnotesize
\setlength{\tabcolsep}{2pt}
\input{generated/tab_v3_gating}
...
\paragraph{Options-only.} Options alone score \vthreeOptDSCRLo{} to \vthreeOptDSCRHi{}\% on DSCR (below the \dscrMajorityPct{}\% majority rate), \vthreeOptPJRFLo{} to \vthreeOptPJRFHi{}\% on PJRF and \vthreeOptTCMLo{} to \vthreeOptTCMHi{}\% on TCM, so options are not the main shortcut there; on LIL (\vthreeOptLILLo{} to \vthreeOptLILHi{}\%) they match the same items with image and stem. AIA is the exception because its answer is the same string in every item. Always answering progressive disease scores \dscrMajorityPct{}\% on DSCR, above every model (\vthreeDscrAccGemmaTwentySeven{}, \vthreeDscrAccScout{}, \vthreeDscrAccGemmaTwelve{} and \vthreeDscrAccMedGemma{}\%), so no model shows RANO skill beyond class imbalance. MedGemma-4B's options-only responses were unparseable for \vthreeMedGemmaUnparsedLo{} to \vthreeMedGemmaUnparsedHi{}\% of items in four phases, so those cells are not interpreted.

\paragraph{Reading.} On \texttt{v3}, removing images changed accuracy by at most \vthreeMaxAbsDiff{} points with every interval including zero, swapped images did not move answers toward the donor beyond a permutation null, and TCM depends on the upstream category its keys encode. The flip rate is not low, so the plan's third condition is not met as written. We do not conclude that the images are ignored: the LIL, DSCR and AIA keys are not functions of the displayed slices, so these items cannot show image dependence, and no clinician has checked the keys.
...
\paragraph{Option-only baselines} (\texttt{v3}, options shuffled, $n=52$). Always choosing the longest option scores \vthreeLongestAIA{}\% on AIA, \vthreeLongestLIL{}\% on LIL, \vthreeLongestDSCR{}\% on DSCR (the majority class, since progressive disease is typically the longest option), \vthreeLongestPJRF{}\% on PJRF and \vthreeLongestTCM{}\% on TCM. The most common correct letter, fitted on the same items, scores \vthreeLetterPriorLo{} to \vthreeLetterPriorHi{}\% on every phase. Except for AIA (a constant correct string) and DSCR (class imbalance), these sit near four-option chance.
